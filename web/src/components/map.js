...
export function renderRun(width, data, callouts = []) {
  ...
  function zoomed(transform) {
    ...
    dots
      .on("mouseenter", function (event, d) {
        ...
        if (d.data.nearest_land_lat && d.data.nearest_land_lon) {
          ...
          if (landPoint && landPoint[0] >= -100 && landPoint[0] <= width + 100 &&
            landPoint[1] >= -100 && landPoint[1] <= height + 100) {
    ...
    calloutGroups.each(function (d) {
      ...
      // Update arrow path
      group.select(".callout-arrow")
        .attr("d", pathData);

      // Update icon position
      group.select(".callout-icon-bg")
        .attr("cx", d.iconX)
        .attr("cy", d.iconY);

      group.select(".callout-icon")
        .attr("x", d.iconX)
        .attr("y", d.iconY)
        .text(d.icon);

      group.select(".callout-hover-target")
        .attr("cx", d.iconX)
        .attr("cy", d.iconY);
    });
    ...
    calloutGroups
      .on("mouseenter", function (event, d) {
        ...
        // Highlight callout
        d3.select(this).select(".callout-icon-bg")
          .attr("fill", "#f0f0f0");
      })
      .on("mouseleave", function () {
        // Remove tooltip
