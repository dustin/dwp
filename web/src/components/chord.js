...
export function renderChord(widthIn, beaches) {
  ...
  for (const { start_beach, end_beach } of beaches)
    matrix[index.get(start_beach)][index.get(end_beach)] += 1;
  ...
  const sources = d3.rollup(
    chords,
    (c) => c.map((x) => x.source.index),
    (c) => c.target.index,
  );
  ...
  const lblTitle = (d) => {
    const launches = d3.sum(
      chords,
      (c) => (c.source.index === d.index) * c.source.value,
    );
    const landings = d3.sum(
      chords,
      (c) => (c.target.index === d.index) * c.source.value,
    );
    let parts = [];
    if (launches > 0) {
      parts.push([`${launches} ${pluralize(launches, "launch", "launches")}`]);
    }
    if (landings > 0) {
      parts.push([`${landings} ${pluralize(landings, "landing", "landings")}`]);
    }
    return parts.join(" ");
  };
  ...
  g.selectAll(".nodes")
    ...
    .attr("opacity", 0.8)
    .on("mouseover", (e, d) => {
      g.selectAll(".nodes").attr("opacity", 0.2);

      g.selectAll(".links").attr("opacity", 0.1);

      g.selectAll(".labels").attr("opacity", 0.2);

      g.select(`#node-${d.index}`).attr("opacity", 0.8);
      g.selectAll(`.target-of-${d.index}`).attr("opacity", 0.8);

      g.selectAll(`.source-link-${d.index}`).attr("opacity", 0.5);

      g.select(`#label-${d.index}`).attr("opacity", 1);
    })
    .on("mouseout", (d) => {
      g.selectAll(".nodes").attr("opacity", 0.8);

      g.selectAll(".links").attr("opacity", 0.3);

      g.selectAll(`.labels`).attr("opacity", 1);
    })
    .append("title")
    .text(lblTitle);
